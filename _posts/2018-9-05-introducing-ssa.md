...
is the property of an [intermediate representation][intermediate] that
each variable is assigned exactly once, and that every variable is
...
This is the head of the function. It gives the module name, function name,
...
Here is the first real instruction! All instructions have this same
...
Note that `@ssa_arity` does not have a numeric suffix, since there is
...
This is another comment added by the pretty printer. It means that the
blocks that follow will never be executed, because there is no block
that will ever branch to them.
...
The code generator always includes block 1 with the exact same instructions in every
function, even if it will never be used.

We will not go into details about the purpose of this block in this
blog post, but just note that that translated back to Erlang it would
look this:
...
split block 3 two blocks. At the end of block 3 there is a variant of
the `br` terminator that we has not seen before.  `br label 10`
